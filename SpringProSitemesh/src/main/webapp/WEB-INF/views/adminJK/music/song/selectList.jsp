...
<table class="table table-hover table-condensed">
	...
	<c:if test="${ listc !=null }">
		<c:forEach var="song" begin="${ pBean.startNum }" end="${ pBean.endNum }" items="${ listc }">
			<tr>
				<td>${ song.num }</td>
				<td><a onclick="goSelOne(${ song.num },'gogogo')">${ song.songName }</a>(${ song.albumName })</td>
				<td>${ song.artistName }</td>
				<td>
					<c:choose>
						<c:when test="${ song.photoSysName != 'none' }">O</c:when>
						<c:otherwise>X</c:otherwise>
					</c:choose>
				</td>
				<td>
					<c:choose>
						<c:when test="${ song.sysName != 'none' }">O</c:when>
						<c:otherwise>X</c:otherwise>
					</c:choose>
				</td>
				<td>
					<c:choose>
						<c:when test="${ song.youtube != null }">O</c:when>
						<c:otherwise>X</c:otherwise>
					</c:choose>
				</td>
				<td>
					<fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${ song.regdate }" />
				</td>
				<td>
					<a onclick="goUpdate(${ song.num },'gogogo')" class="btn btn-info btn-md" ><i class="material-icons">create</i>수정</a>
					<a href="deleteOne?num=${ song.num }" class="btn btn-info btn-md"><i class="material-icons">remove_circle</i>삭제</a>
				</td>
			</tr>
		</c:forEach>
	</c:if>
...
<script type="text/javascript">
 var path = "/springPro/adminJK/music/song";
</script>
