...
<table class="table table-hover table-condensed">
	...
	<c:if test="${ listc !=null }">
		<c:forEach var="info" begin="${ pBean.startNum }" end="${ pBean.endNum }"
			items="${ listc }">
			<tr>
				<td>${ info.num }</td>
				<td>${ info.infoType }</td>
				<td><a onclick="goSelOne(${ info.num },'gogogo')">${ info.title }</a></td>
				<td>
					<a onclick="goUpdate(${ info.num },'gogogo')" class="btn btn-info btn-md"><i class="material-icons">create</i>수정</a>
					<a href="deleteOne?num=${ info.num }" class="btn btn-info btn-md"><i class="material-icons">remove_circle</i>삭제</a>
				</td>
			</tr>
		</c:forEach>
	</c:if>
...
<script type="text/javascript">
 var path = "/springPro/adminJK/customer/info";
</script>
