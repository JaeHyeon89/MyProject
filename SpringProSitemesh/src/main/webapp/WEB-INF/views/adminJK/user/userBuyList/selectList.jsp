...
<table class="table table-hover table-condensed">
	...
	<c:if test="${ listc !=null }">
		<c:forEach var="userBuyList" begin="${ pBean.startNum }" end="${ pBean.endNum }"
			items="${ listc }">
			<tr>
				<td>${ userBuyList.num }</td>
				<td><a onclick="goSelOne(${ uuserBuyListser.num },'gogogo')">${ userBuyList.classify }</a></td>
				<td>
					<a onclick="goUpdate(${ userBuyList.num },'gogogo')" class="btn btn-info btn-md" ><i class="material-icons">create</i>수정</a>
					<a href="deleteOne?num=${ userBuyList.num }" class="btn btn-info btn-md"><i class="material-icons">remove_circle</i>삭제</a>
				</td>
			</tr>
		</c:forEach>
	</c:if>
...
<script type="text/javascript">
 var path = "/springPro/adminJK/user/userBuyList";
</script>
