...
<h2>유저앨범 관리 페이지</h2>
<a onclick="goInsert('gogogo')" class="btn btn-userAlbum btn-sm"><i class="material-icons">add_circle</i>등록</a>
<table class="table table-hover table-condensed">
	...
	<c:if test="${ listc !=null }">
		<c:forEach var="userAlbum" begin="${ pBean.startNum }" end="${ pBean.endNum }"
			items="${ listc }">
			<tr>
				<td>${ userAlbum.num }</td>
				<td><a onclick="goSelOne(${ userAlbum.num },'gogogo')">${ userAlbum.userNum }</a></td>
				<td>
					<a onclick="goUpdate(${ userAlbum.num },'gogogo')" class="btn btn-userAlbum btn-md"><i class="material-icons">create</i>수정</a>
					<a href="deleteOne?num=${ userAlbum.num }" class="btn btn-userAlbum btn-md"><i class="material-icons">remove_circle</i>삭제</a>
				</td>
			</tr>
		</c:forEach>
	</c:if>
...
<script type="text/javascript">
 var path = "/springPro/adminJK/user/userAlbum";
</script>
