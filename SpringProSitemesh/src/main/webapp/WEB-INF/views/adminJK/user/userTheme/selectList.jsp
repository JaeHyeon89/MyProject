...
<table class="table table-hover table-condensed">
	...
	<c:if test="${ listc !=null }">
		<c:forEach var="userTheme" begin="${ pBean.startNum }" end="${ pBean.endNum }"
			items="${ listc }">
			<tr>
				<td>${ userTheme.num }</td>
				<td><a onclick="goSelOne(${ userTheme.num },'gogogo')">${ userTheme.userNum }</a></td>
				<td>
					<a onclick="goUpdate(${ userTheme.num },'gogogo')" class="btn btn-info btn-md" ><i class="material-icons">create</i>수정</a>
					<a href="deleteOne?num=${ userTheme.num }" class="btn btn-info btn-md"><i class="material-icons">remove_circle</i>삭제</a>
				</td>
			</tr>
		</c:forEach>
	</c:if>
...
<script type="text/javascript">
 var path = "/springPro/adminJK/user/userTheme";
</script>
